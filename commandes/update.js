...
zokou({
    ...
    if (!superUser) return repondre("❌ *𝐓𝐄𝐃𝐃-𝐗𝐌𝐃:* Unauthorized access. Only the Owner can trigger updates.");

    repondre("🔄 *𝐓𝐄𝐃𝐃-𝐗𝐌𝐃 Update System:* Connecting to GitHub...");

    const repoUrl = "https://github.com/Teddytech1/Cteddy";
    ...
    exec(updateScript, (err, stdout, stderr) => {
        ...
        if (stdout.includes("Already up to date") || stderr.includes("Already up to date")) {
            return repondre("✅ *𝐓𝐄𝐃𝐃-𝐗𝐌𝐃* is already running the latest version from GitHub.");
        }
        ...
        exec("npm install", (installErr, installStdout, installStderr) => {
            ...
            repondre("🚀 *TEDDY-XMD Updated!* System is restarting to apply changes...");
