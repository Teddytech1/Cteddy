...
const thumb = "https://files.catbox.moe/13nyhx.jpg";
...
zokou({ nomCom: "photo", categorie: "Conversion", reaction: "📸" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        ...
        exec(`ffmpeg -i ${mediaPath} ${outPath}`, async (err) => {
            ...
            await zk.sendMessage(dest, { image: fs.readFileSync(outPath), caption: "*Converted by TEDDY-XMD*" }, { quoted: ms });
...
zokou({ nomCom: "url", aliases: ["tourl"], categorie: "Conversion", reaction: "🔗" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        ...
        await zk.sendMessage(dest, {
            text: `*𝚃𝙴𝙳𝙳𝚈-𝚇𝙼𝙳 𝚄𝚁𝙻 𝚄𝙿𝙻𝙾𝙰𝙳𝙴𝚁*\n\n🔗 *URL:* ${url}`,
            contextInfo: {
                forwardingScore: 999,
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: channelJid,
                    newsletterName: "𝚃𝙴𝙳𝙳𝚈-𝚇𝙼𝙳 𝙲𝙾𝙽𝚅𝙴𝚁𝚃𝙴𝚁",
                    serverMessageId: 1
                }
            }
        }, { quoted: ms });
